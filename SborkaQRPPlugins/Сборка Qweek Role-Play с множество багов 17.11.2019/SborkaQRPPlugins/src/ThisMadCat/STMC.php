...
use Frago9876543210\EasyForms\elements\custom\StepSlider;
...
class STMC extends PluginBase implements Listener{
    ...
    function onEnable()
    {
        ...
        $this->easyforms = $this->getServer()->getPluginManager()->getPlugin('EasyForms');
      }
      ...
      public function onLol(BlockBreakEvent $e){ //Приват
        $p = $e->getPlayer();
      ...
      public function onKek(BlockPlaceEvent $e){ // Приват
        $p = $e->getPlayer();
        $a = $this->admins->getAll();
        if (isset($a["admin"][$p->getName()])) {
          if ($a["admin"][$p->getName()] < 3){
      ...
      public function onChatpgsffffe(PlayerChatEvent $e) {
        $sender = $e->getPlayer();
        if($e->getMessage(){0} == "(") {
          $name = $sender->getName();
          $admins = $this->admins->getAll();
            ...
            if ($x == $signX && $y == $signY && $z == $signZ) {
              if ($data[$i]['owner'] == 'not') {
                ...
                if ($this->players->get($event->getPlayer()->getName()) == NULL) {
                  if ($ra["mon"][$event->getPlayer()] >= $data[$i]['price']) {
      ...
      public function onCommand(CommandSender $sender, Command $command, String $label, array $args) :bool{
        ...
        if($command->getName() == "showmc"){
          ...
          $date = $this->cares->getAll();
          if(isset($args[0])){
            if($med[$player->getName()]["o"] == "Есть"){
              if($player != NULL){
                if($sender->distance($player->asVector3()) < 6){
                  $player->sendMessage("§b| §cМедицинская Карта §3" . $sender->getName());
                  $player->sendMessage("§b| §cСостояние человека: §3" . $med[$sender->getName()]["sos"]);
                  $player->sendMessage("§b| §cСостояние здоровья: §3" .$date[$sender->getName()]["pills"] . "%");
                }else{
                  $sender->sendMessage("§7Данный игрок далеко от вас.");
                  return false;
                }
              }else{
                $sender->sendMessage("§7Данный игрок не на севрере.");
                return false;
              }
            }else{
              $sender->sendMessage("§7У вас не имеется медицинской карты");
              return false;
            }
          }else{
            $sender->sendMessage("§7Использование: /showmc <nick>");
